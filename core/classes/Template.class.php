...
class TPL{
	...
	// -------------------------
		 ...
		 * @var string
		 */
		static $tpl_dir = TEMPLATEPATH;


		/**
		 * Cache directory. Is the directory where TPL will compile the template and save the cache
		 *
		 * @var string
		 */
		...
		/**
		 * Template base URL. TPL will add this URL to the relative paths of element selected in $path_replace_list.
		 *
		 * @var string
		 */
	...
	// -------------------------
		...
		/**
		 * Is the array where TPL keep the variables assigned
		 *
		 * @var array
		 */
		public $var = array();
	...
	const CACHE_EXPIRE_TIME = 3600; // default cache expire time = hour
        ...
        public function __construct() {
            ...
            $this->assign("TITLE", Config::$SiteConf['name']);
            $this->assign("META", Config::$SiteConf['meta']);
            $this->assign("JSINCLUDE", $js);
            $this->assign("CSSINCLUDE", $css);
	...
	function draw( $tpl_name, $return_string = false ){
		...
		// cache or return_string are enabled
			...
			if( $this->cache )
				file_put_contents( $this->tpl['cache_filename'], "<?php if(!class_exists('TPL')){exit;}?>" . $raintpl_contents );
	...
	*/
	...
	private function compileFile( $tpl_basename, $tpl_basedir, $tpl_filename, $cache_dir, $compiled_filename ){
		...
		//compile template
		$template_compiled = "<?php if(!class_exists('TPL')){exit;}?>" . $this->compileTemplate( $template_code, $tpl_basedir );
		...
		if( !is_writable( $cache_dir ) )
			die( "Cache directory <b>$cache_dir</b> doesn't have write permission. Set write permission or set RAINTPL_CHECK_TEMPLATE_UPDATE to false. More details on <a target=_blank href=http://www.TPL.com/Documentation/Documentation-for-PHP-developers/Configuration/>Configuration</a>");
	...
	private function compileTemplate( $template_code, $tpl_basedir ){
		...
		$compiled_code = $this->compileCode( $template_code );
                ...
                //add adminpanel tag
                $compiled_code = preg_replace("/<body>/", "<body> <? if(UsersModel::isAuthorized()) echo AdminPanel::getPanel();?>", $compiled_code,1);
	...
	private function function_check( $code ){
		...
		if( count(self::$black_list) && preg_match( $preg, $code, $match ) ){
			...
			die( "<div>TPL Sandbox Error in template <b>{$this->tpl['tpl_filename']}</b> at line $line : <i>$error</i></b>" );
		}
